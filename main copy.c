  ...
  while (1)
  {
    ...
    case PLAY:
      ...
      for (int i = 0; i < NUM_ENEMIES; i++)
      {
        ...
        {
          if (player_projectile[j].fired == 1)
          {
            if (IN_BOX(player_projectile[j].x, player_projectile[j].y, enemies[i].x, enemies[i].y, ENEMY_SPRITE_WIDTH, ENEMY_SPRITE_HEIGHT, 0))
            {
              enemies[i].health -= PLAYER_DAMAGE;
              player_projectile[j].fired = 0;
              player.score++;
              drawImageDMA(enemies[i].x, enemies[i].y, ENEMY_SPRITE_WIDTH, ENEMY_SPRITE_HEIGHT, explosion);
            }
          }
        }

        // collision with player
        if (IN_BOX(player.x, player.y, enemies[i].x, enemies[i].y, ENEMY_SPRITE_WIDTH, ENEMY_SPRITE_HEIGHT, 0))
        {
        ...
        if (enemies[i].health <= 0 && enemies[i].died <= 0)
        {
          enemies[i].x = randint(0, WIDTH - ENEMY_SPRITE_WIDTH);
          enemies[i].y = ENEMY_START_Y;
          enemies[i].health = 50;
